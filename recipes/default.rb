...
user node[:grinder][:user] do
  comment 'The Grinder System User'
  system true
  gid node[:grinder][:group]
  action :create
end
...
remote_file zip_path do
  ...
  # Create if missing doesn't correctly handle zero byte file size
  action :create
  retries 5
  not_if { File.size?(zip_path) }
end
...
bash 'extract_grinder' do
  ...
  code <<-EOH
    ...
    chown -R #{node[:grinder][:user]}:#{node[:grinder][:group]} "$(echo grinder-#{node.grinder.version}*)"
  EOH
end
